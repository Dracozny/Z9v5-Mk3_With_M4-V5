...
#define	CONFIG_TUNE_CASE_TOTAL (ENABLED(FWRETRACT)+ENABLED(OPTION_REPEAT_PRINTING)+ENABLED(FILAMENT_RUNOUT_SENSOR)+ENABLED(POWER_LOSS_RECOVERY)+ ENABLED(OPTION_AUTOPOWEROFF)+ENABLED(CASE_LIGHT_MENU)+ENABLED(OPTION_WIFI_MODULE))
#define	CONFIG_CASE_TOTAL (CONFIG_TUNE_CASE_TOTAL + BOTH(OPTION_WIFI_MODULE, OPTION_WIFI_BAUDRATE) + ENABLED(SWITCH_EXTRUDER_MENU) + BOTH(MIXING_EXTRUDER, OPTION_MIXING_SWITCH) + ENABLED(OPTION_BED_COATING) + ENABLED(OPTION_HOTENDMAXTEMP) + EITHER(PID_EDIT_MENU, PID_AUTOTUNE_MENU) + ABL_GRID*2)
